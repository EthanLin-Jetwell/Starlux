({
    ...
    ProcessFormFields : function (component, event, helper) {
        ...
        try {
            ...
            //Medical Reasons
            var medicalValue = component.get("v.medicalValue");
            var multiSelectValue = '';
            for (var i =0; i<medicalValue.length;i++) {
                if (i < medicalValue.length-1) {
                	multiSelectValue += medicalValue[i];
                	multiSelectValue += ';';        
                } else {
                    multiSelectValue += medicalValue[i];
                }
            }
            eventFields["Medical_Reason__c"] = multiSelectValue;
            eventFields["Description"] = "Medical Reasons: " + multiSelectValue;
